# -*- coding: utf-8 -*-
"""
Complete Journal Analysis Pipeline (with correct DDC split + meaning labels)

What this script produces (in OUTPUT_DIR):
1) population_dynamics_active_turnover.png/pdf
   - Panel A: total active journals over time
   - Panel B: births (+), deaths (-), net (dashed)
2) turnover_data.xlsx
3) genealogy_families.xlsx
4) 3_genealogy_gantt.png
5) ddc_subject_group_code_counts.xlsx
6) ddc_subject_group_code_counts_top.png
7) (LAST) 1_timeline_area_chart_ddc_codes.png/pdf  <-- stacked area by split DDC codes, legend includes meaning

Key constraints:
- Read zdb_journal_results.xlsx
- Filter: Type of publication == "Periodical (Journal)"
- DDC subject groups:
    * split multi-values (e.g., "000; 020") into separate codes
    * count each code
    * stacked area uses split codes (not raw strings), legend shows code + meaning
- No guessing beyond official/fallback mapping for code meanings
"""
