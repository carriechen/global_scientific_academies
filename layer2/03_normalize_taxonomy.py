...
def main() -> None:
    ...
    df_url.to_excel(output_dir / "dfurlhier_normalize.xlsx", index=False)
    df_menu.to_excel(output_dir / "dfmenu_normalize.xlsx", index=False)
    ...
    taxonomy_df.to_excel(output_dir / "taxonomy.xlsx", index=False)
    md = to_markdown_taxonomy(taxonomy_df)
    (output_dir / "unified_taxonomy.md").write_text(md, encoding="utf-8")

    print(f"saved: {output_dir / 'dfurlhier_normalize.xlsx'}")
    print(f"saved: {output_dir / 'dfmenu_normalize.xlsx'}")
    print(f"saved: {output_dir / 'hierdata_refine.xlsx'}")
    print(f"saved: {output_dir / 'taxonomy.xlsx'}")
