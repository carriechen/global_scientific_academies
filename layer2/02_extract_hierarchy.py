...
def main() -> None:
    ...
    parser.add_argument("--site-list", required=True, help="national_academy_web_mapping.xlsx")
    parser.add_argument("--menu-category", required=True, help="title categorization")
    parser.add_argument("--url-tags", required=True, help="url_tags")
    ...
    df110 = df[df["sitedomain"].isin(sitedomains)].copy()
    df110.reset_index(drop=True).to_feather(output_dir / "df110dom.feather")

    dfurlhier110 = dfurlhier[dfurlhier["domain"].isin(sitedomains)].copy()
    dfurlhier110.to_feather(output_dir / "dfurlhier110.feather")
    ...
    dfurlhier110["ifwords"] = dfurlhier110[["Name", "Parent"]].apply(
        lambda x: (str(x.iloc[0]) in url_tags) or (str(x.iloc[1]) in url_tags), axis=1
    )
    refined = dfurlhier110[dfurlhier110["ifwords"]].copy()
    ...
    refined.to_excel(output_dir / "dfurlhier110Refine.xlsx", index=False)

    # Menu-based hierarchy using title categories
    df110menu = df110[~df110["title_origin"].isna()].copy()
    ...
    df110menu = df110menu.merge(dfmenu, on=merge_cols, how="left")
    if "Response" not in df110menu.columns:
        raise ValueError("menu category file must contain a Response column")

    menu_hier = derive_menu_hierarchy(df110menu)
    menu_hier = menu_hier.merge(df110[["site_name", "sitedomain"]].drop_duplicates(), on="site_name", how="left")
    menu_hier.to_excel(output_dir / "dfmenu110hier.xlsx", index=False)
    ...
    print(f"saved: {output_dir / 'dfurlhier110Refine.xlsx'}")
    print(f"saved: {output_dir / 'dfmenu110hier.xlsx'}")
